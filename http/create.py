...
def addTasks(file):
    try:
        ...
        for task in json['tasks']:
            ...
            _put = "        self.client.post('%s', data=%s, headers=%s)\n\n" % (task['path'] , task['data'],task['headers'] )
...
parser.add_argument('--build-project', type=str, default="", help='Build a project')
args = parser.parse_args()
print(args.build_project)
for filename in findProject('project', '*.json'):
    if filename.split("/")[1] == args.build_project:
        print(filename)
        addTasks(filename)
